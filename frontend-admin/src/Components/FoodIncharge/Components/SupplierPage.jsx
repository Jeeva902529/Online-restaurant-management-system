...
const SupplierPage = () => {
  ...
  const selectSupplier = (supplier) => {
    setSelectedSupplier(selectedSupplier?.name === supplier.name ? null : supplier)
  }
  ...
  const confirmAssign = async () => {
    try {
      ...
      const orderId = orderList && orderList.length > 0 ? orderList[0]._id : null
      ...
      const response = await fetch("http://localhost:5000/api/suppliers/send-notification", {
        method: "POST",
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || errorData.details || "Notification failed")
      }
  ...
  return (
    <div className="bg-gray-100 text-gray-900 min-h-screen p-6 relative">
      ...
      <div className={`flex flex-col gap-6 items-center transition-all duration-300 ${isBlurred ? "blur-sm" : ""}`}>
        <div className="text-center">
          ...
          {tableNumber && (
            <div className="bg-white p-4 rounded-lg shadow-md mb-6">
              ...
              <p className="text-gray-700">Total Amount: Rs. {totalAmount || 0}</p>
            </div>
          )}
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-14 w-full max-w-4xl">
          ...
          <div className="w-full bg-white p-3 rounded-lg shadow-lg max-h-65 overflow-y-auto">
            <div className="space-y-2">
              {suppliers.filter((s) => s.status === "Available").length === 0 && (
                <p className="text-center py-4 text-gray-500">No available suppliers</p>
              )}
              {suppliers
                ...
                .map((supplier) => (
                  ...
                  >
                    ...
                    <button
                      ...
                      disabled={isLoading}
                    >
                      {selectedSupplier?.name === supplier.name ? "Selected" : "Assign"}
                    </button>
                  </div>
                ))}
            </div>
          ...
          <div className="w-full bg-white p-3 rounded-lg shadow-lg max-h-65 overflow-y-auto">
            <div className="space-y-2">
              {suppliers.filter((s) => s.status === "Busy").length === 0 && (
                <p className="text-center py-4 text-gray-500">No busy suppliers</p>
              )}
              {suppliers
                ...
                .map((supplier) => (
                  ...
                  >
                    <span className="text-gray-700 font-medium text-lg">{supplier.name}</span>
                    <span className="px-4 py-2 rounded-lg bg-green-500 text-white">Assigned</span>
                  </div>
                ))}
            </div>
          </div>
        ...
        <div className="w-full max-w-4xl bg-white p-3 rounded-lg shadow-lg max-h-40 overflow-y-auto">
          <div className="space-y-2">
            {suppliers.filter((s) => s.status === "Absent").length === 0 && (
              <p className="text-center py-4 text-gray-500">No absent suppliers</p>
            )}
            {suppliers
        ...
        >
          ...
          >
            <Dialog.Panel className="bg-white p-8 rounded-lg shadow-xl max-w-sm w-full text-center z-50">
              <Dialog.Title className="text-2xl font-semibold text-gray-800 mb-2">Confirm Assignment</Dialog.Title>
              <Dialog.Description className="text-gray-600 mb-4">
                Are you sure you want to mark <span className="font-bold">{selectedSupplier?.name}</span> as Busy?
              </Dialog.Description>
