...
const Suppliers = () => {
  ...
  useEffect(() => {
    ...
    if (searchQuery.trim() !== "") {
      ...
      filtered = filtered.filter(
        (supplier) => supplier.name.toLowerCase().includes(query) || supplier.supplierId.toLowerCase().includes(query),
      )
    }

    // Apply status filter
    if (filterStatus !== "all") {
      filtered = filtered.filter((supplier) => supplier.attendance.toLowerCase() === filterStatus)
    }
  ...
  const toggleSupplierAttendance = async (supplier) => {
    const newAttendance = supplier.attendance === "Present" ? "Absent" : "Present"

    setIsLoading(true)
    try {
      await axios.patch(`${API_BASE}/${supplier._id}`, { attendance: newAttendance })

      // Update local state immediately for better UX
      setSuppliers((prevSuppliers) =>
        prevSuppliers.map((s) =>
          s._id === supplier._id
            ? { ...s, attendance: newAttendance, status: newAttendance === "Absent" ? "Absent" : "Available" }
            : s,
        ),
      )
  ...
  const handleCreateSupplier = async (e) => {
    ...
    // Validation
    if (!supplierForm.name || !supplierForm.supplierId || !supplierForm.password) {
      setMessage({
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-400 text-sm mt-1">Daily attendance resets automatically at 12:00 AM</p>
        </div>
      ...
      {showCreateForm && (
        <div className="bg-gray-800 p-6 rounded-lg border border-gray-700">
          ...
          <form onSubmit={handleCreateSupplier} className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-300 mb-2">Initial Attendance</label>
              <select
                value={supplierForm.attendance}
                onChange={(e) => setSupplierForm({ ...supplierForm, attendance: e.target.value })}
                className="w-full bg-gray-700 border border-gray-600 rounded-lg px-3 py-2 text-white focus:outline-none focus:ring-2 focus:ring-blue-500"
              >
              ...
              >
                {isLoading ? (
                  <>
                    ...
                    <span>{editingSupplier ? "Updating..." : "Creating..."}</span>
                  </>
                ) : (
                  <>
      ...
      {/* Suppliers Grid */}
      <div className="bg-gray-800 p-6 rounded-lg border border-gray-700">
        <h3 className="text-xl font-bold text-white mb-4">Suppliers List ({filteredSuppliers.length})</h3>
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredSuppliers.map((supplier) => (
              <div key={supplier._id} className="bg-gray-700 p-6 rounded-lg border border-gray-600">
                <div className="flex justify-between items-start mb-4">
                  <div className="flex items-start space-x-3">
                    ...
                    <div>
                      ...
                      <p className="text-gray-400 text-sm">ID: {supplier.supplierId}</p>
                  ...
                  <div className="flex items-center space-x-2">
                    <span
                      className={`text-sm font-medium ${
                        supplier.attendance === "Present" ? "text-green-400" : "text-red-400"
                      }`}
                    >
                      {supplier.attendance}
                    </span>
                    <button
                      ...
                      className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                        supplier.attendance === "Present" ? "bg-green-600" : "bg-gray-600"
                      } ${isLoading ? "opacity-50 cursor-not-allowed" : ""}`}
                    >
                      <span
                        className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${
                          supplier.attendance === "Present" ? "translate-x-6" : "translate-x-1"
                        }`}
                      />
                    </button>
                  </div>
      ...
      <div className="bg-gray-800 p-4 rounded-lg border border-gray-700">
        <div className="flex items-center space-x-2 text-gray-400">
          ...
          <span className="text-sm">Automatic Reset: All suppliers will be marked as "Present" daily at 12:00 AM</span>
        </div>
        {lastResetDate && <p className="text-xs text-gray-500 mt-1">Last reset: {lastResetDate}</p>}
      </div>
    </div>
  )
}
