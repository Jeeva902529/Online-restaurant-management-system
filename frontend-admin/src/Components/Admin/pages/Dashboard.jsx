  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      const [ordersRes, suppliersRes, menuRes] = await Promise.allSettled([
        ...
        axios.get("http://localhost:5000/api/foods/noodles"), // You can modify this to get all categories
      ])
  ...
  const generateWeeklyTrend = (orders) => {
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      weekData.push({
        ...
        totalAmount: dayOrders.reduce((sum, order) => sum + (order.totalPrice || 0), 0),
      })
    }
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="bg-white dark:bg-gray-800 p-6 rounded-lg border border-gray-200 dark:border-gray-700">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-gray-900 dark:text-white">{dashboardData.totalOrders}</p>
            </div>
        ...
        <div className="bg-white dark:bg-gray-800 p-6 rounded-lg border border-gray-200 dark:border-gray-700">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-gray-600 dark:text-gray-400 text-sm">Menu Items</p>
              <p className="text-2xl font-bold text-gray-900 dark:text-white">{dashboardData.totalMenuItems}</p>
            </div>
            <UtensilsCrossed className="h-8 w-8 text-green-500" />
          </div>
          <p className="text-blue-500 text-sm mt-2 flex items-center">
            <ChefHat className="h-3 w-3 mr-1" />
            {dashboardData.menuStats.veg} veg, {dashboardData.menuStats.nonVeg} non-veg
          </p>
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 p-6 rounded-lg border border-gray-200 dark:border-gray-700">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-gray-900 dark:text-white">{dashboardData.activeSuppliers}</p>
            </div>
        ...
        <div className="bg-white dark:bg-gray-800 p-6 rounded-lg border border-gray-200 dark:border-gray-700">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-gray-900 dark:text-white">{dashboardData.lowStockItems.length}</p>
            </div>
              ...
              dashboardData.recentOrders.map((order) => (
                ...
                >
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      <p className="font-medium text-gray-900 dark:text-white">#{order._id.slice(-6)}</p>
                      <p className="text-sm text-gray-600 dark:text-gray-400">{order.foodName}</p>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className="font-bold text-green-600">{formatCurrency(order.totalPrice)}</p>
                    <p className="text-xs text-gray-500 dark:text-gray-400">{timeAgo(order.createdAt)}</p>
                  </div>
                </div>
              ))
            )}
            ...
            dashboardData.recentActivity.map((activity) => (
              <div key={activity.id} className="flex items-center space-x-3">
                ...
                <div className="flex-1">
                  ...
                  <span className="text-gray-500 dark:text-gray-400 text-sm">{timeAgo(activity.time)}</span>
                </div>
              </div>
            ))
          )}
      ...
      <div className="bg-white dark:bg-gray-800 p-6 rounded-lg border border-gray-200 dark:border-gray-700">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="flex items-center space-x-3">
            <div className={`h-5 w-5 rounded-full ${autoRefresh ? "bg-green-500" : "bg-gray-400"}`}></div>
            <div>
              <p className="font-medium text-gray-900 dark:text-white">Auto Refresh</p>
              <p className={`text-sm ${autoRefresh ? "text-green-600" : "text-gray-500"}`}>
