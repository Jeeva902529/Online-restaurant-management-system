...
import { Save, Eye, EyeOff, RefreshCw, CheckCircle, AlertCircle, Users, UserPlus } from "lucide-react"
...
const Registration = () => {
  ...
  const handleRegistrationSubmit = async (e) => {
    ...
    // Validation
    if (!registrationForm.username || !registrationForm.password || !registrationForm.role) {
      setMessage({
  ...
  const getRolePermissions = (role) => {
    const permissions = {
      cashier: ["Process orders", "View menu items", "Handle payments", "Basic reporting"],
      supplier: ["Update inventory", "View orders", "Manage deliveries", "Status updates"],
      foodincharge: ["Manage menu items", "Recipe management", "Quality control", "Kitchen operations"],
      admin: ["Full system access", "User management", "Analytics & reports", "System settings"],
    }
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-400 text-sm mt-1">Create and manage user accounts for the restaurant system</p>
        </div>
        ...
        <form onSubmit={handleRegistrationSubmit} className="space-y-6">
          ...
          <div>
            ...
            <p className="text-xs text-gray-400 mt-1">
              Selected role permissions: {getRolePermissions(registrationForm.role).join(", ")}
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-4">
              <div>
                ...
                />
                <p className="text-xs text-gray-400 mt-1">This will be used for login</p>
              </div>
              ...
              {registrationForm.password && (
                <div className="space-y-1">
                  ...
                  <div className="flex space-x-1">
                    <div
                      className={`h-1 w-full rounded ${registrationForm.password.length >= 6 ? "bg-green-500" : "bg-gray-600"}`}
                    ></div>
                    <div
                      className={`h-1 w-full rounded ${registrationForm.password.length >= 8 ? "bg-green-500" : "bg-gray-600"}`}
                    ></div>
                    <div
                      className={`h-1 w-full rounded ${/[A-Z]/.test(registrationForm.password) ? "bg-green-500" : "bg-gray-600"}`}
                    ></div>
                    <div
                      className={`h-1 w-full rounded ${/[0-9]/.test(registrationForm.password) ? "bg-green-500" : "bg-gray-600"}`}
                    ></div>
                  </div>
                </div>
              )}
      ...
      <div className="bg-gray-800 p-6 rounded-lg border border-gray-700">
        <h3 className="text-xl font-semibold text-white mb-4">Role Permissions Overview</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {["cashier", "supplier", "foodincharge", "admin"].map((role) => (
            <div key={role} className="bg-gray-700 p-4 rounded-lg">
              <h4 className={`font-semibold mb-2 capitalize ${getRoleColor(role)}`}>
