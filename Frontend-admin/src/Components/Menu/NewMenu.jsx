...
const NewMenu = ({ isOpen, onClose, onAddDish, existingCategories }) => {
  ...
  const [dishType, setDishType] = useState("veg"); // new dish type state
  ...
  const handleAdd = () => {
    ...
    if (finalSection.trim() && dishName.trim() && image && price.trim()) {
      onAddDish(finalSection, dishName, image, price, dishType);
      setSection("");
      setNewSection("");
  ...
  return (
    <div className="fixed inset-0 backdrop-blur-md flex justify-center items-center z-50">
      <div className="bg-white p-6 rounded-lg shadow-lg w-96 relative">
        ...
        <button
          className={`mt-4 w-full px-4 py-2 rounded text-white ${(section || newSection) && dishName && image && price ? "bg-blue-500 hover:bg-blue-700" : "bg-gray-300 cursor-not-allowed"}`}
          onClick={handleAdd}
          disabled={!(section || newSection) || !dishName || !image || !price}
        >
          Add Dish
        </button>
      </div>
