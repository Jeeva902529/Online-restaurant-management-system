...
const MenuPage = () => {
  ...
  // ✅ Add Dish Function (includes dishType)
  const handleAddDish = async (section, dishName, image, price, dishType) => {
    // Create an optimistic dish object with a temporary ID.
    const tempDish = {
    ...
    // Optimistically update UI: add the new dish to the correct category.
    setMenuCategories((prevCategories) => {
      const updated = [...prevCategories];
      const catIndex = updated.findIndex((cat) => cat.category === section);
      if (catIndex >= 0) {
        // Category exists; update based on dish type.
        ...
        // Category doesn't exist; create a new category entry.
        updated.push(
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `Server error: ${response.status} ${response.statusText} - ${errorData.error}`
        );
      }
      alert("Dish added successfully!");
      // Re-fetch the menu to ensure UI is in sync with the database.
      fetchMenu();
    } catch (error) {
      console.error("Error adding dish:", error);
      alert("Failed to add dish. Error: " + error.message);
      // Revert optimistic update by re-fetching the menu.
      fetchMenu();
    }
  };
  ...
  const handleUpdateDish = async (updatedDish) => {
    ...
    try {
      ...
      fetchMenu(); // Refresh menu in case of failure
    }
  };
                  ...
                  {catObj.vegItems.map((item) => (
                    ...
                    >
                      ...
                      <div className="flex justify-around w-full mt-2">
                        ...
                        <button
                          onClick={() => setDishToDelete({ ...item, category: catObj.category })} // Pass dish category
        ...
        <div className="fixed inset-0 backdrop-blur-md flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-xl p-6 w-80">
            ...
            <div className="flex justify-end gap-4">
              <button
                onClick={() => setDishToDelete(null)} // Cancel deletion
