...
import biryani from "../assets/biryani-menu.jpg"

export default function BiryaniMenu() {
  const [biryaniVarieties, setBiryaniVarieties] = useState([])
  const [selectedBiryani, setSelectedBiryani] = useState(null)
  const [isModalOpen, setIsModalOpen] = useState(false)
  const [addOns, setAddOns] = useState([
    { name: "RAITA", price: 10, quantity: 0 },
    { name: "EXTRA GRAVY", price: 15, quantity: 0 },
    { name: "BOILED EGG", price: 20, quantity: 0 },
    { name: "FRIED ONIONS", price: 12, quantity: 0 },
    { name: "MINT CHUTNEY", price: 8, quantity: 0 },
    { name: "EXTRA SPICY", price: 5, quantity: 0 },
    { name: "EXTRA CHICKEN", price: 40, quantity: 0 },
    { name: "EXTRA MUTTON", price: 60, quantity: 0 },
  ])
  ...
  // Fetch biryani data from API
  useEffect(() => {
    axios
      .get("http://localhost:5000/api/biryani")
      .then((response) => setBiryaniVarieties(response.data))
  ...
  const calculateTotal = () => {
    ...
    return selectedBiryani ? selectedBiryani.price + addOnsTotal : 0
  }
  ...
  const addToCart = () => {
    if (selectedBiryani) {
      ...
      const addOnsTotal = addOns.reduce((sum, addon) => sum + addon.price * addon.quantity, 0)
      const totalPrice = selectedBiryani.price + addOnsTotal
      const orderData = {
        foodName: selectedBiryani.name,
        basePrice: selectedBiryani.price,
        addOns: addOns
    ...
    <div className="flex min-h-screen flex-col md:flex-row">
      ...
      <div className="relative h-[40vh] md:h-screen md:w-1/2 lg:w-3/5">
        <div className="absolute inset-0 bg-gradient-to-t from-black/70 to-black/20 flex items-end p-8">
          <div>
            <h1 className="text-5xl font-bold text-white mb-2">BIRYANI</h1>
            <p className="text-xl text-gray-200">Aromatic rice dish with exotic spices</p>
          </div>
        </div>
        <img src={biryani} alt="Biryani Dish" className="object-cover w-full h-full" />
      </div>
      ...
      <div className="flex flex-1 flex-col bg-gray-50 md:h-screen md:overflow-hidden">
        <div className="sticky top-0 bg-gray-50 p-6 pb-4 z-10 shadow-sm">
          ...
          <h2 className="text-3xl font-bold">
            <span className="text-[#ff3131]">Flavors of India</span> <span className="text-[#122348]">Biryani</span>
          </h2>
          <p className="text-gray-600 mt-1">Select your favorite biryani from our authentic collection</p>
        </div>
        <div className="flex-1 overflow-y-auto p-6 pt-2">
          <div className="grid gap-4">
            {biryaniVarieties.map((item, index) => (
              <div
                key={index}
          ...
          <DialogContent className="bg-white max-w-md md:max-w-3xl rounded-xl shadow-lg p-0 overflow-hidden">
            <div className="flex flex-col md:flex-row">
              ...
              <div className="relative hidden md:block md:w-2/5 h-auto bg-[#122348]">
                <img
                  src={biryani}
                  alt={selectedBiryani?.name}
                  className="w-full h-full object-cover opacity-80"
                />
                <div className="absolute inset-0 flex items-center justify-center">
                  <h2 className="text-3xl font-bold text-white text-center px-4">{selectedBiryani?.name} Biryani</h2>
                </div>
              </div>
              {/* Right side - Content */}
              <div className="p-4 md:p-5 w-full md:w-3/5">
                ...
                <div className="md:hidden mb-3">
                  <h2 className="text-xl font-bold text-[#122348]">
                    {selectedBiryani?.name} <span className="text-[#ff3131]">Biryani</span>
                  </h2>
                </div>
                <p className="text-gray-600 text-sm italic mb-3">{selectedBiryani?.description}</p>
                <div className="mb-3">
