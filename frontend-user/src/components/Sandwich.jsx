...
import sandwichImage from "../assets/sandwichmenu.jpg" // Replace with actual image path

export default function SandwichMenu() {
  const [sandwichVarieties, setSandwichVarieties] = useState([])
  const [selectedSandwich, setSelectedSandwich] = useState(null)
  const [isModalOpen, setIsModalOpen] = useState(false)
  const [addOns, setAddOns] = useState([
    { name: "EXTRA CHEESE", price: 15, quantity: 0 },
    { name: "SPICY SAUCE", price: 10, quantity: 0 },
    { name: "VEGGIES", price: 20, quantity: 0 },
    { name: "GRILLED CHICKEN", price: 40, quantity: 0 },
    { name: "BACON", price: 30, quantity: 0 },
    { name: "EGG", price: 25, quantity: 0 },
  ])
  const [customNotes, setCustomNotes] = useState("")
  const [cartItems, setCartItems] = useState([])
  const navigate = useNavigate()

  // Fetch Sandwich data from API
  useEffect(() => {
    axios
      .get("http://localhost:5000/api/sandwich") // Updated API endpoint
      .then((response) => setSandwichVarieties(response.data))
    ...
    <div className="flex min-h-screen flex-col md:flex-row">
      ...
      <div className="relative h-[40vh] md:h-screen md:w-1/2 lg:w-3/5">
        <div className="absolute inset-0 bg-gradient-to-t from-black/70 to-black/20 flex items-end p-8">
          <div>
            <h1 className="text-5xl font-bold text-white mb-2">SANDWICH</h1>
            <p className="text-xl text-gray-200">Delicious layers of flavor in every bite</p>
          </div>
        </div>
      ...
      <div className="flex flex-1 flex-col bg-gray-50 md:h-screen md:overflow-hidden">
        <div className="sticky top-0 bg-gray-50 p-6 pb-4 z-10 shadow-sm">
          ...
          <h2 className="text-3xl font-bold">
            <span className="text-[#ff3131]">Flavors of the World</span> <span className="text-[#122348]">Sandwich</span>
          </h2>
          <p className="text-gray-600 mt-1">Select your favorite sandwich from our authentic collection</p>
        </div>
        <div className="flex-1 overflow-y-auto p-6 pt-2">
          <div className="grid gap-4">
            {sandwichVarieties.map((item, index) => (
              <div
                key={index}
