...
import pieImage from "../assets/piemenu.jpg" // Replace with actual image path

export default function PieMenu() {
  const [pieVarieties, setPieVarieties] = useState([])
  const [selectedPie, setSelectedPie] = useState(null)
  const [isModalOpen, setIsModalOpen] = useState(false)
  const [addOns, setAddOns] = useState([
    { name: "EXTRA CRUST", price: 15, quantity: 0 },
    { name: "WHIPPED CREAM", price: 12, quantity: 0 },
    { name: "ICE CREAM SCOOP (VANILLA)", price: 25, quantity: 0 },
    { name: "ICE CREAM SCOOP (CHOCOLATE)", price: 25, quantity: 0 },
  ...
  useEffect(() => {
    axios
      .get("http://localhost:5000/api/pie") // Updated API endpoint
      .then((response) => setPieVarieties(response.data))
    ...
    <div className="flex min-h-screen flex-col md:flex-row">
      ...
      <div className="relative h-[40vh] md:h-screen md:w-1/2 lg:w-3/5">
        <div className="absolute inset-0 bg-gradient-to-t from-black/70 to-black/20 flex items-end p-8">
          <div>
            <h1 className="text-5xl font-bold text-white mb-2">PIE</h1>
            <p className="text-xl text-gray-200">Deliciously baked with love</p>
          </div>
        </div>
      ...
      <div className="flex flex-1 flex-col bg-gray-50 md:h-screen md:overflow-hidden">
        <div className="sticky top-0 bg-gray-50 p-6 pb-4 z-10 shadow-sm">
          ...
          <h2 className="text-3xl font-bold">
            <span className="text-[#ff3131]">Flavors of Sweetness</span> <span className="text-[#122348]">Pie</span>
          </h2>
          <p className="text-gray-600 mt-1">Select your favorite pie from our delightful collection</p>
        </div>
        <div className="flex-1 overflow-y-auto p-6 pt-2">
          <div className="grid gap-4">
            {pieVarieties.map((item, index) => (
              <div
                key={index}
            ...
            <div className="flex flex-col md:flex-row">
              ...
              <div className="p-4 md:p-5 w-full md:w-3/5">
                ...
                <div className="mb-3">
                  ...
                  <Textarea
                    placeholder="Any special requests? (e.g., no whipped cream, extra caramel)"
                    value={customNotes}
