...
export default function NoodlesMenu() {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:5000/api/noodles") // Updated API endpoint
  ...
  const addToCart = () => {
    ...
    if (tokenParts.length !== 3) {
      console.error("Error: Token is invalid or malformed!", token);
      return;
    }
  
    let decodedToken;
    try {
      decodedToken = jwtDecode(token); // ✅ Proper decoding
      if (!decodedToken.tableNumber) {
        console.error("Error: Table number missing in token!", decodedToken);
        return;
      }
    } catch (error) {
    ...
    const orderData = {
      ...
      tableNumber: decodedToken.tableNumber, // ✅ Safely extracted
    };
