...
import doughnutImage from "../assets/donutsmenu.jpg" // Replace with actual image path

export default function DoughnutMenu() {
  const [doughnutVarieties, setDoughnutVarieties] = useState([])
  const [selectedDoughnut, setSelectedDoughnut] = useState(null)
  const [isModalOpen, setIsModalOpen] = useState(false)
  const [addOns, setAddOns] = useState([
    { name: "CHOCOLATE GLAZE", price: 15, quantity: 0 },
    { name: "SPRINKLES", price: 10, quantity: 0 },
    { name: "POWDERED SUGAR", price: 8, quantity: 0 },
    { name: "CARAMEL DRIZZLE", price: 12, quantity: 0 },
  ...
  // Fetch Doughnut data from API
  useEffect(() => {
    axios
      .get("http://localhost:5000/api/doughnuts") // Updated API endpoint
      .then((response) => setDoughnutVarieties(response.data))
      ...
      <div className="relative h-[40vh] md:h-screen md:w-1/2 lg:w-3/5">
        <div className="absolute inset-0 bg-gradient-to-t from-black/70 to-black/20 flex items-end p-8">
          <div>
            ...
            <p className="text-xl text-gray-200">Soft, sweet, and utterly delightful</p>
          </div>
        </div>
        <img src={doughnutImage} alt="Doughnut Dish" className="object-cover w-full h-full" />
      </div>
      {/* Menu Section */}
      <div className="flex flex-1 flex-col bg-gray-50 md:h-screen md:overflow-hidden">
        <div className="sticky top-0 bg-gray-50 p-6 pb-4 z-10 shadow-sm">
          ...
          <h2 className="text-3xl font-bold">
            <span className="text-[#ff3131]">Flavors of Sweetness</span> <span className="text-[#122348]">Doughnuts</span>
          </h2>
          <p className="text-gray-600 mt-1">Select your favorite doughnuts from our delightful collection</p>
        </div>
        <div className="flex-1 overflow-y-auto p-6 pt-2">
          <div className="grid gap-4">
            {doughnutVarieties.map((item, index) => (
              <div
                key={index}
            ...
            <div className="flex flex-col md:flex-row">
              ...
              <div className="p-4 md:p-5 w-full md:w-3/5">
                ...
                <div className="mb-3">
                  ...
                  <Textarea
                    placeholder="Any special requests? (e.g., no sprinkles, extra chocolate)"
                    value={customNotes}
