...
import brownieImage from "../assets/browniesmenu.jpg" // Replace with actual image path

export default function BrownieMenu() {
  const [brownieVarieties, setBrownieVarieties] = useState([])
  const [selectedBrownie, setSelectedBrownie] = useState(null)
  const [isModalOpen, setIsModalOpen] = useState(false)
  const [addOns, setAddOns] = useState([
    { name: "EXTRA CHOCOLATE CHUNKS", price: 15, quantity: 0 },
    ...
    { name: "FRESH BERRIES", price: 20, quantity: 0 },
  ...
  // Fetch Brownie data from API
  useEffect(() => {
    axios
      .get("http://localhost:5000/api/brownies") // Updated API endpoint
      .then((response) => setBrownieVarieties(response.data))
    ...
    <div className="flex min-h-screen flex-col md:flex-row">
      ...
      <div className="relative h-[40vh] md:h-screen md:w-1/2 lg:w-3/5">
        <div className="absolute inset-0 bg-gradient-to-t from-black/70 to-black/20 flex items-end p-8">
          <div>
            <h1 className="text-5xl font-bold text-white mb-2">BROWNIE</h1>
            <p className="text-xl text-gray-200">Rich, gooey, and utterly irresistible</p>
          </div>
        </div>
        <img src={brownieImage} alt="Brownie Dish" className="object-cover w-full h-full" />
      </div>
      {/* Menu Section */}
      <div className="flex flex-1 flex-col bg-gray-50 md:h-screen md:overflow-hidden">
        <div className="sticky top-0 bg-gray-50 p-6 pb-4 z-10 shadow-sm">
          ...
          <h2 className="text-3xl font-bold">
            <span className="text-[#ff3131]">Flavors of Sweetness</span> <span className="text-[#122348]">Brownie</span>
          </h2>
          <p className="text-gray-600 mt-1">Select your favorite brownie from our delightful collection</p>
        </div>
        <div className="flex-1 overflow-y-auto p-6 pt-2">
          <div className="grid gap-4">
            {brownieVarieties.map((item, index) => (
              <div
                key={index}
