...
import burgerImage from "../assets/burgermenu.jpg" // Replace with actual image path

export default function BurgerMenu() {
  const [burgerVarieties, setBurgerVarieties] = useState([])
  const [selectedBurger, setSelectedBurger] = useState(null)
  const [isModalOpen, setIsModalOpen] = useState(false)
  const [addOns, setAddOns] = useState([
    { name: "EXTRA CHEESE", price: 15, quantity: 0 },
    { name: "BACON", price: 25, quantity: 0 },
    { name: "AVOCADO", price: 30, quantity: 0 },
    { name: "JALAPEÑOS", price: 10, quantity: 0 },
    { name: "SPICY SAUCE", price: 8, quantity: 0 },
  ])
  ...
  useEffect(() => {
    axios
      .get("http://localhost:5000/api/burger") // Updated API endpoint
      .then((response) => setBurgerVarieties(response.data))
    ...
    <div className="flex min-h-screen flex-col md:flex-row">
      ...
      <div className="relative h-[40vh] md:h-screen md:w-1/2 lg:w-3/5">
        <div className="absolute inset-0 bg-gradient-to-t from-black/70 to-black/20 flex items-end p-8">
          <div>
            <h1 className="text-5xl font-bold text-white mb-2">BURGER</h1>
            <p className="text-xl text-gray-200">Juicy patties with bold flavors</p>
          </div>
        </div>
      ...
      <div className="flex flex-1 flex-col bg-gray-50 md:h-screen md:overflow-hidden">
        <div className="sticky top-0 bg-gray-50 p-6 pb-4 z-10 shadow-sm">
          ...
          <h2 className="text-3xl font-bold">
            <span className="text-[#ff3131]">Flavors of America</span> <span className="text-[#122348]">Burger</span>
          </h2>
          <p className="text-gray-600 mt-1">Select your favorite burger from our authentic collection</p>
        </div>
        <div className="flex-1 overflow-y-auto p-6 pt-2">
          <div className="grid gap-4">
            {burgerVarieties.map((item, index) => (
              <div
                key={index}
