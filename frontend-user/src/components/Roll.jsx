...
import rollImage from "../assets/rollmenu.jpg" // Replace with actual image path

export default function RollMenu() {
  const [rollVarieties, setRollVarieties] = useState([])
  ...
  const [addOns, setAddOns] = useState([
    ...
    { name: "CHICKEN", price: 30, quantity: 0 },
    { name: "PANEER", price: 25, quantity: 0 },
  ])
  const [customNotes, setCustomNotes] = useState("")
  const [cartItems, setCartItems] = useState([])
  const navigate = useNavigate()

  // Fetch Roll data from API
  useEffect(() => {
    axios
      .get("http://localhost:5000/api/roll") // Updated API endpoint
      .then((response) => setRollVarieties(response.data))
      ...
      <div className="relative h-[40vh] md:h-screen md:w-1/2 lg:w-3/5">
        <div className="absolute inset-0 bg-gradient-to-t from-black/70 to-black/20 flex items-end p-8">
          <div>
            ...
            <p className="text-xl text-gray-200">Delicious wraps with bold flavors</p>
          </div>
        </div>
      ...
      <div className="flex flex-1 flex-col bg-gray-50 md:h-screen md:overflow-hidden">
        <div className="sticky top-0 bg-gray-50 p-6 pb-4 z-10 shadow-sm">
          ...
          <h2 className="text-3xl font-bold">
            <span className="text-[#ff3131]">Flavors of Street Food</span> <span className="text-[#122348]">Rolls</span>
          </h2>
          <p className="text-gray-600 mt-1">Select your favorite roll from our authentic collection</p>
        </div>
        <div className="flex-1 overflow-y-auto p-6 pt-2">
          <div className="grid gap-4">
            {rollVarieties.map((item, index) => (
              <div
                key={index}
