...
import cakeImage from "../assets/cakemenu.jpg" // Replace with actual image path

export default function CakeMenu() {
  const [cakeVarieties, setCakeVarieties] = useState([])
  const [selectedCake, setSelectedCake] = useState(null)
  const [isModalOpen, setIsModalOpen] = useState(false)
  const [addOns, setAddOns] = useState([
    { name: "EXTRA CHOCOLATE TOPPING", price: 25, quantity: 0 },
    { name: "WHIPPED CREAM", price: 15, quantity: 0 },
    { name: "FRESH FRUITS", price: 20, quantity: 0 },
    { name: "CANDLES", price: 8, quantity: 0 },
    { name: "CUSTOM MESSAGE", price: 50, quantity: 0 },
  ])
  ...
  useEffect(() => {
    axios
      .get("http://localhost:5000/api/cake") // Updated API endpoint
      .then((response) => setCakeVarieties(response.data))
    ...
    <div className="flex min-h-screen flex-col md:flex-row">
      ...
      <div className="relative h-[40vh] md:h-screen md:w-1/2 lg:w-3/5">
        <div className="absolute inset-0 bg-gradient-to-t from-black/70 to-black/20 flex items-end p-8">
          <div>
            <h1 className="text-5xl font-bold text-white mb-2">CAKE</h1>
            <p className="text-xl text-gray-200">Sweet indulgence with every bite</p>
          </div>
        </div>
        <img src={cakeImage} alt="Cake Dish" className="object-cover w-full h-full" />
      </div>
      {/* Menu Section */}
      <div className="flex flex-1 flex-col bg-gray-50 md:h-screen md:overflow-hidden">
        <div className="sticky top-0 bg-gray-50 p-6 pb-4 z-10 shadow-sm">
          ...
          <h2 className="text-3xl font-bold">
            <span className="text-[#ff3131]">Flavors of Sweetness</span> <span className="text-[#122348]">Cake</span>
          </h2>
          <p className="text-gray-600 mt-1">Select your favorite cake from our delightful collection</p>
        </div>
        <div className="flex-1 overflow-y-auto p-6 pt-2">
          <div className="grid gap-4">
            {cakeVarieties.map((item, index) => (
              <div
                key={index}
            ...
            <div className="flex flex-col md:flex-row">
              ...
              <div className="p-4 md:p-5 w-full md:w-3/5">
                ...
                <div className="mb-3">
                  ...
                  <Textarea
                    placeholder="Any special requests? (e.g., custom message, no candles)"
                    value={customNotes}
