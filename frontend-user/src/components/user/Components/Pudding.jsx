...
import puddingImage from "../assets/puddingmenu.jpg" // Replace with actual image path

export default function PuddingMenu() {
  const [puddingVarieties, setPuddingVarieties] = useState([])
  ...
  const [addOns, setAddOns] = useState([
    ...
    { name: "NUTS (ALMONDS, WALNUTS)", price: 25, quantity: 0 },
    { name: "VANILLA ICE CREAM SCOOP", price: 30, quantity: 0 },
  ])
  const [customNotes, setCustomNotes] = useState("")
  const [cartItems, setCartItems] = useState([])
  const navigate = useNavigate()

  // Fetch Pudding data from API
  useEffect(() => {
    axios
      .get("http://localhost:5000/api/pudding") // Updated API endpoint
      .then((response) => setPuddingVarieties(response.data))
    ...
    <div className="flex min-h-screen flex-col md:flex-row">
      ...
      <div className="relative h-[40vh] md:h-screen md:w-1/2 lg:w-3/5">
        <div className="absolute inset-0 bg-gradient-to-t from-black/70 to-black/20 flex items-end p-8">
          <div>
            <h1 className="text-5xl font-bold text-white mb-2">PUDDING</h1>
            <p className="text-xl text-gray-200">Silky smooth desserts with rich flavors</p>
          </div>
        </div>
        <img src={puddingImage} alt="Pudding Dish" className="object-cover w-full h-full" />
      </div>
      {/* Menu Section */}
      <div className="flex flex-1 flex-col bg-gray-50 md:h-screen md:overflow-hidden">
        <div className="sticky top-0 bg-gray-50 p-6 pb-4 z-10 shadow-sm">
          ...
          <h2 className="text-3xl font-bold">
            <span className="text-[#ff3131]">Flavors of Sweetness</span> <span className="text-[#122348]">Pudding</span>
          </h2>
          <p className="text-gray-600 mt-1">Select your favorite pudding from our delightful collection</p>
        </div>
        <div className="flex-1 overflow-y-auto p-6 pt-2">
          <div className="grid gap-4">
            {puddingVarieties.map((item, index) => (
              <div
                key={index}
