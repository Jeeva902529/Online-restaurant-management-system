...
import iceCreamImage from "../assets/icecreammenu.jpg" // Replace with actual image path

export default function IceCreamMenu() {
  const [iceCreamVarieties, setIceCreamVarieties] = useState([])
  ...
  const [addOns, setAddOns] = useState([
    ...
    { name: "NUTS", price: 20, quantity: 0 },
    { name: "WHIPPED CREAM", price: 12, quantity: 0 },
    { name: "HOT FUDGE", price: 25, quantity: 0 },
    { name: "CARAMEL DRIZZLE", price: 20, quantity: 0 },
    { name: "FRESH FRUITS", price: 18, quantity: 0 },
  ])
  const [customNotes, setCustomNotes] = useState("")
  const [cartItems, setCartItems] = useState([])
  const navigate = useNavigate()

  // Fetch Ice Cream data from API
  useEffect(() => {
    axios
      .get("http://localhost:5000/api/icecream") // Updated API endpoint
      .then((response) => setIceCreamVarieties(response.data))
      ...
      <div className="relative h-[40vh] md:h-screen md:w-1/2 lg:w-3/5">
        <div className="absolute inset-0 bg-gradient-to-t from-black/70 to-black/20 flex items-end p-8">
          <div>
            ...
            <p className="text-xl text-gray-200">Chilled delights in every scoop</p>
          </div>
        </div>
        <img src={iceCreamImage} alt="Ice Cream Dish" className="object-cover w-full h-full" />
      </div>
      {/* Menu Section */}
      <div className="flex flex-1 flex-col bg-gray-50 md:h-screen md:overflow-hidden">
        <div className="sticky top-0 bg-gray-50 p-6 pb-4 z-10 shadow-sm">
          ...
          <h2 className="text-3xl font-bold">
            <span className="text-[#ff3131]">Flavors of Sweetness</span> <span className="text-[#122348]">Ice Cream</span>
          </h2>
          <p className="text-gray-600 mt-1">Select your favorite ice cream from our delightful collection</p>
        </div>
        <div className="flex-1 overflow-y-auto p-6 pt-2">
          <div className="grid gap-4">
            {iceCreamVarieties.map((item, index) => (
              <div
                key={index}
            ...
            <div className="flex flex-col md:flex-row">
              ...
              <div className="relative hidden md:block md:w-2/5 h-auto bg-[#122348]">
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                  <h2 className="text-3xl font-bold text-white text-center px-4">{selectedIceCream?.name} Ice Cream</h2>
                </div>
              </div>
              {/* Right side - Content */}
              <div className="p-4 md:p-5 w-full md:w-3/5">
                ...
                <div className="mb-3">
                  ...
                  <Textarea
                    placeholder="Any special requests? (e.g., no nuts, extra toppings)"
                    value={customNotes}
