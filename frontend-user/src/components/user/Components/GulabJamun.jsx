...
import gulabJamunImage from "../assets/gulabjamun.jpg" // Replace with actual image path

export default function GulabJamunMenu() {
  const [gulabJamunVarieties, setGulabJamunVarieties] = useState([])
  ...
  useEffect(() => {
    axios
      .get("http://localhost:5000/api/gulab-jamun") // Updated API endpoint
      .then((response) => setGulabJamunVarieties(response.data))
      ...
      <div className="relative h-[40vh] md:h-screen md:w-1/2 lg:w-3/5">
        <div className="absolute inset-0 bg-gradient-to-t from-black/70 to-black/20 flex items-end p-8">
          <div>
            ...
            <p className="text-xl text-gray-200">Soft, juicy, and soaked in sweet syrup</p>
          </div>
        </div>
        <img src={gulabJamunImage} alt="Gulab Jamun Dish" className="object-cover w-full h-full" />
      </div>
      {/* Menu Section */}
      <div className="flex flex-1 flex-col bg-gray-50 md:h-screen md:overflow-hidden">
        <div className="sticky top-0 bg-gray-50 p-6 pb-4 z-10 shadow-sm">
          ...
          <h2 className="text-3xl font-bold">
            <span className="text-[#ff3131]">Flavors of Sweetness</span> <span className="text-[#122348]">Gulab Jamun</span>
          </h2>
          <p className="text-gray-600 mt-1">Select your favorite Gulab Jamun from our delightful collection</p>
        </div>
        <div className="flex-1 overflow-y-auto p-6 pt-2">
          <div className="grid gap-4">
            {gulabJamunVarieties.map((item, index) => (
              <div
                key={index}
            ...
            <div className="flex flex-col md:flex-row">
              ...
              <div className="p-4 md:p-5 w-full md:w-3/5">
                ...
                <div className="mb-3">
                  ...
                  <Textarea
                    placeholder="Any special requests? (e.g., less syrup, extra ice cream)"
                    value={customNotes}
