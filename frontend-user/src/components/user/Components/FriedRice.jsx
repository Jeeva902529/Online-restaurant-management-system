...
import friedRiceImage from "../assets/friedrice.jpg" // Replace with actual image path

export default function FriedRiceMenu() {
  const [friedRiceVarieties, setFriedRiceVarieties] = useState([])
  const [selectedFriedRice, setSelectedFriedRice] = useState(null)
  const [isModalOpen, setIsModalOpen] = useState(false)
  const [addOns, setAddOns] = useState([
    { name: "EXTRA VEGETABLES", price: 15, quantity: 0 },
    { name: "BOILED EGG", price: 20, quantity: 0 },
    { name: "FRIED EGG", price: 25, quantity: 0 },
    { name: "CHILI FLAKES", price: 8, quantity: 0 },
    { name: "SOY SAUCE", price: 10, quantity: 0 },
    { name: "EXTRA CHICKEN", price: 40, quantity: 0 },
    { name: "EXTRA SHRIMP", price: 50, quantity: 0 },
  ])
  ...
  useEffect(() => {
    axios
      .get("http://localhost:5000/api/fried-rice") // Updated API endpoint
  ...
  const calculateTotal = () => {
    ...
    return selectedFriedRice ? selectedFriedRice.price + addOnsTotal : 0
  }

  const addToCart = () => {
    if (selectedFriedRice) {
      const token = localStorage.getItem("token")
      const orderData = {
        foodName: selectedFriedRice.name,
        basePrice: selectedFriedRice.price,
        addOns: addOns
    ...
    <div className="flex min-h-screen flex-col md:flex-row">
      ...
      <div className="relative h-[40vh] md:h-screen md:w-1/2 lg:w-3/5">
        <div className="absolute inset-0 bg-gradient-to-t from-black/70 to-black/20 flex items-end p-8">
          <div>
            ...
            <p className="text-xl text-gray-200">A flavorful blend of rice, vegetables, and spices</p>
          </div>
        </div>
      ...
      <div className="flex flex-1 flex-col bg-gray-50 md:h-screen md:overflow-hidden">
        <div className="sticky top-0 bg-gray-50 p-6 pb-4 z-10 shadow-sm">
          ...
          <p className="text-gray-600 mt-1">Select your favorite fried rice from our authentic collection</p>
        </div>
          ...
          <DialogContent className="bg-white max-w-md md:max-w-3xl rounded-xl shadow-lg p-0 overflow-hidden">
            <div className="flex flex-col md:flex-row">
              ...
              <div className="relative hidden md:block md:w-2/5 h-auto bg-[#122348]">
                <img
                  src={friedRiceImage}
                  alt={selectedFriedRice?.name}
                  className="w-full h-full object-cover opacity-80"
                />
                <div className="absolute inset-0 flex items-center justify-center">
                  <h2 className="text-3xl font-bold text-white text-center px-4">{selectedFriedRice?.name} Fried Rice</h2>
                </div>
              </div>
              {/* Right side - Content */}
              <div className="p-4 md:p-5 w-full md:w-3/5">
                ...
                <div className="md:hidden mb-3">
                  <h2 className="text-xl font-bold text-[#122348]">
                    {selectedFriedRice?.name} <span className="text-[#ff3131]">Fried Rice</span>
                  </h2>
                </div>
                <p className="text-gray-600 text-sm italic mb-3">{selectedFriedRice?.description}</p>
                <div className="mb-3">
                ...
                <div className="mb-3">
                  ...
                  <Textarea
                    placeholder="Any special requests? (e.g., less spicy, extra shrimp)"
                    value={customNotes}
